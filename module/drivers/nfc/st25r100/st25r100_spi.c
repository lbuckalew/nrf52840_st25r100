...
{
    const struct nfc_cfg *config = dev->config;
...
void platform_st25r_spi_deselect()
{
    ...
    const struct nfc_cfg *config = s_spi_dev->config;
    spi_release_dt(&config->spi_bus);
}
...
void platform_st25r_spi_transceive(const uint8_t *txBuf, uint8_t *rxBuf, uint16_t len)
{
    ...
    if (spi_transceive_dt(&config->spi_bus, &tx, &rx))
    {
